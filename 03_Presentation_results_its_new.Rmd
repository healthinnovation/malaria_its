---
title: "03 Presentation analysis"
author: "Et al"
date: "11-20-2023"
output:
  rmdformats::robobook:
...
db_vx_pfro                <-  readRDS("db_frcst_bestmodels_vx_pfro.rds")
...
db.full.prov.error.tbl_vx_pfro <- db.prov.error.tbl_vx_pfro %>%
                  ...
                  # rename_with(~str_detect(.,'^PROPHET$'), 'prophet') %>% 
                  # rename_with(~str_detect(.,'ARIMA'), 'arima') %>% 
                  # rename_with(~str_detect(.,'NNAR'), 'nnar')
...
db_flpm_pfro             <-  readRDS("db_frcst_bestmodels_flcpm_pfro.rds")
...
db_vx_mcero             <-  readRDS("db_frcst_bestmodels_vx_mcero.rds")
...
db_flpm_mcero             <-  readRDS("db_frcst_bestmodels_flcpm_mcero.rds")
...
# 
# 
# other_metrics_st <- db.error.tbl |>
#                     multi_metric(PF,fitted_spatio)|>
#                     pluck(".estimate")
# 
# other_metrics_fe <- db.error.tbl |>
...
geom_ribbon(aes( x   =  .index,
                 ...
                 ymax = .conf_hi,
                fill=  .model_desc),alpha=0.5)                  +
scale_fill_manual("Model",values = c("#231E23","darkgoldenrod2"))                 +   
scale_color_manual("Model",values = c("#231E23","darkgoldenrod2"))                +
geofacet::facet_geo(~ id, grid = loreto_grid, label = "name")   +
theme(  
      axis.line = element_line(color = "black"),
      axis.line.x = element_line(colour = "black"), 
...
geom_ribbon(aes( x   =  .index,
                 ...
                 ymax = .conf_hi,
                fill=  .model_desc),alpha=0.5)                  +
scale_fill_manual("Model",values = c("black","darkgoldenrod2"))                 +   
scale_color_manual("Model",values = c("black","darkgoldenrod2"))                +
geofacet::facet_geo(~ id, grid = loreto_grid, label = "name")   +
theme(  
      axis.line = element_line(color = "black"),
      axis.line.x = element_line(colour = "black"), 
...
scale_fill_gradient2(low = ocean.curl(7)[2]
                               ...
                               "Variacion\nde casos\n(O-E)",
                     breaks = c(-70000,-50000,-30000,-10000,0)) +
theme_bw()
...
scale_fill_gradient2(low = ocean.curl(7)[2]
                                , mid = ocean.curl(7)[4] , high =ocean.curl(7)[6] , midpoint = 0,
                               "Variacion\nde casos\n(O-E)",
                     breaks = c(-20000,-16000,-12000,-8000,-4000,-1200,0,1200)
                     ) +
theme_bw()
...
scale_fill_gradient2(low = colorspace::diverge_hcl(7)[1],
                               ...
                               "Variacion\nde casos\n(O-E)",
                     breaks = c(-1000,0,1000,4000,7000,10000)) +
theme_bw()
